...
describe("Wallet Aggregate Root", () => {
  ...
  test("throws DomainError with INSUFFICIENT_FUNDS and does not mutate wallet when debit exceeds balance", () => {
    ...
    expect(() =>
      wallet.debit(Money.create("0.01", "BRL"), "tx-3", "entry-3"),
    ).toThrow(DomainError);
    expect(wallet.balance.amount).toBe("0.00");
    expect(wallet.version).toBe(1);
  });
  ...
  test("throws DomainError when attempting to credit/debit zero or with differing currency", () => {
    ...
    expect(() => wallet.credit(Money.zero("BRL"), "tx-4", "entry-4")).toThrow(
      DomainError,
    );
    expect(() =>
      wallet.credit(Money.create("10.00", "USD"), "tx-5", "entry-5"),
    ).toThrow(DomainError);
    expect(() =>
      wallet.debit(Money.create("10.00", "USD"), "tx-6", "entry-6"),
    ).toThrow(DomainError);
  });
