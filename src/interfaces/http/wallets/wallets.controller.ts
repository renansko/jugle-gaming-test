...
export class WalletsController {
  ...
  public async reconcile(
    ...
    try {
      return await this.reconciliation.reconcile(walletId);
    } catch (error) {
      if (error instanceof DomainError && error.code === "WALLET_NOT_FOUND") {
  ...
  public async create(
    ...
    const validatedInput = createWalletSchema.parse(body);

    try {
      const createdWallet = await this.wallets.create(validatedInput);
      return this.serializeWallet(createdWallet);
    } catch (error) {
      if (
  ...
  public async ledger(
    ...
    let ledgerPage: Awaited<ReturnType<WalletService["ledger"]>>;

    try {
      ledgerPage = await this.wallets.ledger(walletId, cursor, parsedLimit);
    } catch (error) {
      if (error instanceof DomainError && error.code === "INVALID_CURSOR") {
        throw new BadRequestException({
          code: error.code,
          message: error.message,
        });
    ...
    return {
      items: ledgerPage.items.map((entry) => ({
        ...entry,
